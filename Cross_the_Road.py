...
class Game:
    ...
    def run_game_loop(self, level_speed):
        ...
        # Pattern (image_path, x, y, width, height)
        ...
        if did_win:
            ...
            self.run_game_loop(level_speed) # todo
...
class PlayerCharacter(GameObject):
    ...
    def detect_collision(self, other_body):
        # when this fails there is collision on the x pos
        # if the players y pos is lower down than the lowest point of the other body there is no collision
        if self.y_pos > other_body.y_pos + other_body.height:
            return False
        # if the players y pos is above the point of the other body there is no collision
        elif self.y_pos + self.height < other_body.y_pos:
            return False

        # when this fails there is collision on the y pos
        # if the current player x position is greater than the other bodies xpos + width theres no collision
        if self.x_pos > other_body.x_pos + other_body.width:
            return False
        # if the current player position is smalelr than the other bodies xpos + width theres no collision
